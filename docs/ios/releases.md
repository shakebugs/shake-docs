...
[Chat](ios/shake-ui/chat-screen.md), the most upvoted feature on our feedback list is now available!
...
We've introduced [Home screen](/ios/shake-ui/home-screen.md) where [your users](/ios/users/overview.md) can see tickets they reported.
...
From now, you can [register users of your application](/ios/users/overview.md) with Shake. You can assign metadata to each user what
...
If you are using the shaking gesture as your invocation event, you can now set [how sensitive the device should be to the shaking](/ios/user-feedback/invoke.md#shaking).
...
We've added an option to [exclude the screenshot from the reports](/ios/configuration-and-data/auto-screenshot.md#excluding-the-screenshot-from-a-report), this option allows you to capture the screenshot only when required.
...
Screenshots are cool, but wouldn't it be even cooler if you could watch a short video of what a user did before reporting the bug? That's why we introduced [Automatic screen recording](/ios/configuration-and-data/auto-screen-recording.md).
...
You can now use the in-app Shake [onboarding dialog](ios/user-feedback/intro-message.md) to inform your users about Shake availability.
...
and introduce [Metadata](/ios/configuration-and-data/ticket-metadata.md) (useful key-value pairs) instead. So we did it.
...
There’s no such thing as too many [invocation methods](/ios/user-feedback/invoke.md). Shake can now be invoked by a one-finger swiping gesture from the right edge of the screen too.
...
If you want to, our [Activity history](/ios/configuration-and-data/activity-history.md) now allows you to send yourself any custom logs, and read the notifications users receive.
...
By popular demand, Shake can now be [invoked through code](/ios/user-feedback/invoke.md) as well.

Submit yourself a [silent bug report](/ios/user-feedback/silent-user-feedback.md) from the background,
