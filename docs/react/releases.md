...
[Chat](react/shake-ui/chat-screen.md), the most upvoted feature on our feedback list is now available!
...
We've introduced [Home screen](/react/shake-ui/home-screen.md) where [your users](/react/users/overview.md) can see tickets they reported.
...
and introduce [Metadata](/react/configuration-and-data/ticket-metadata.md) (useful key-value pairs) instead. So we did it.
...
If you want to, our [Activity history](/react/configuration-and-data/activity-history.md) now allows you to send yourself any custom logs, and read the notifications users receive.

Users can finally report [different types of feedback](/react/user-feedback/feedback-type.md): bugs, suggestions and questions.

Screenshots are cool, but wouldn't it be even cooler if you could watch a short video of what a user did before reporting the bug? That's why we introduced [Automatic screen recording](/react/configuration-and-data/auto-screen-recording.md).
...
You can now use the in-app Shake [onboarding dialog](react/user-feedback/intro-message.md) to inform your users about Shake availability.
...
By popular demand, Shake can now be [invoked through code](/react/user-feedback/invoke.md) as well.

Submit yourself a [silent bug report](/react/user-feedback/silent-user-feedback.md) from the background,
