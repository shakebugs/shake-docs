...
[Chat](react/shake-ui/chat-screen.md), the most popular feature on the Public feedback board, is now available.
...
We've completely [redesigned Shake](/react/shake-ui/home-screen.md) to continue delivering you more and more options in a scalable manner.
It also started supporting dark and light themes, so it can beautifully fit into your app in either case.

We introduced a [Home screen](/react/shake-ui/home-screen.md) where [your users](/react/users/overview.md) can see tickets they previously reported.
...
From now on, you will be able to [register your app users](/react/users/overview.md) with Shake. You can assign metadata to each user too.
...
If you are using the shaking gesture as your invocation event, you can now [adjust the device’s sensitivity to the shaking gesture](/react/user-feedback/invoke.md#shaking).
 
Shake now supports [customization of feedback types](/react/user-feedback/feedback-type.md#custom-types). You can configure Shake to display any number of custom categories related to your product.

We've added an option to [exclude the screenshot from tickets](/react/configuration-and-data/auto-screenshot.md#excluding-the-screenshot-from-a-report). This option allows you to capture the screenshot only when required.
...
If you’re upgrading from the older version, make sure to [update the dependency in the build.gradle file](/react/install/installation.md#add-shake-dependency-to-your-app-level-buildgradle-file). 
...
Shake now requires `compileSdkVersion` 29 or higher, make sure to [update the version in the build.gradle file](/react/install/installation.md/#set-compilesdkversion-version-in-the-buildgradle-file).

Shake will now [automatically redact sensitive data](/react/configuration-and-data/manage-sensitive-data/#automatically-redacted-sensitive-data) from your network requests, notifications and touch events so it never reaches Shake servers.

We've added [console logs](react/configuration-and-data/activity-history.md#console-logs) to Activity history.
...
Shake is available in the new Maven repository. If you're upgrading from the older version, make sure to [update the Maven repository in the build.gradle file](/react/install/installation.md#add-maven-repository-to-your-top-level-buildgradle-file).

Users can finally report [different types of feedback](/react/user-feedback/feedback-type.md): bugs, suggestions and questions.

Shake now allows you to [protect sensitive data](react/configuration-and-data/manage-sensitive-data.md) on the mobile device itself, so it never reaches the Shake servers. It can be used for notifications, network requests, screens and screen elements.

You can now use the in-app [intro message](react/user-feedback/intro-message.md) to inform your users about Shake availability.
...
If you want to, you can now pre-fill or hide the [Email field](/react/configuration-and-data/new-ticket-screen-elements/#email-field) from the New ticket screen. Pretty neat.
...
Screenshots are cool, but wouldn't it be even cooler if you could watch a short video of what a user did before reporting the ticket? That's why we introduced [Auto screen recording](/react/configuration-and-data/auto-screen-recording.md).
...
and introduce [Ticket metadata](/react/configuration-and-data/ticket-metadata.md) (useful key-value pairs) instead. So we did it.
...
There’s no such thing as too many [invocation methods](/react/user-feedback/invoke.md).
...
If you want to, Shake's [Activity history](/react/configuration-and-data/activity-history.md) now allows you to send yourself any custom logs,
...
Submit yourself [silent user feedback](/react/user-feedback/silent-user-feedback.md) from the app background,
without interrupting your end user whatsoever!

Privacy options are important. That’s why you now have a method to
disable or enable [Activity history](/react/configuration-and-data/activity-history.md) whenever you want to.
