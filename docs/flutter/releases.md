...
[Chat](/flutter/shake-ui/chat-screen.md), the most popular feature on the Public feedback board, is now available.
...
We've completely [redesigned Shake](/flutter/shake-ui/home-screen.md) to continue delivering you more and more options in a scalable manner.
It also started supporting dark and light themes, so it can beautifully fit into your app in either case.

We introduced a [Home screen](/flutter/shake-ui/home-screen.md) where [your users](/flutter/users/overview.md) can see tickets they previously reported.
...
From now on, you will be able to [register your app users](/flutter/users/overview.md) with Shake. You can assign metadata to each user too.
...
If you are using the shaking gesture as your invocation event, you can now [adjust the device’s sensitivity to the shaking gesture](/flutter/user-feedback/invoke.md#shaking).
 
Shake now supports [customization of feedback types](/flutter/user-feedback/feedback-type.md#custom-types). You can configure Shake to display any number of custom categories related to your product.

We've added an option to [exclude the screenshot from tickets](/flutter/configuration-and-data/auto-screenshot.md#excluding-the-screenshot-from-a-report). This option allows you to capture the screenshot only when required.
...
If you’re upgrading from the older version, make sure to [update the dependency in the build.gradle file](/flutter/installation.md#add-shake-dependency-to-your-app-level-buildgradle-file). 
...
Shake now requires `compileSdkVersion` 29 or higher, make sure to [update the version in the build.gradle file](/flutter/installation.md#set-compilesdkversion-version-in-the-buildgradle-file).

Shake will now [automatically redact sensitive data](/flutter/configuration-and-data/manage-sensitive-data/#automatically-redacted-sensitive-data) from your network requests, notifications and touch events so it never reaches Shake servers.

We've added [console logs](/flutter/configuration-and-data/activity-history.md#console-logs) to Activity history.
...
Shake is available in the new Maven repository. If you're upgrading from the older version, make sure to [update the Maven repository in the build.gradle file](/flutter/installation.md#add-maven-repository-to-your-top-level-buildgradle-file).

Users can finally report [different types of feedback](/flutter/user-feedback/feedback-type.md): bugs, suggestions and questions.

Shake now allows you to [protect sensitive data](flutter/configuration-and-data/manage-sensitive-data.md) on the mobile device itself, so it never reaches the Shake servers. It can be used for notifications, network requests, screens and screen elements.

You can now use the in-app [intro message](flutter/user-feedback/intro-message.md) to inform your users about Shake availability.
...
If you want to, you can now pre-fill or hide the [Email field](/flutter/configuration-and-data/new-ticket-screen-elements/#email-field) from the New ticket screen. Pretty neat.
...
Screenshots are cool, but wouldn't it be even cooler if you could watch a short video of what a user did before reporting the ticket? That's why we introduced [Auto screen recording](/flutter/configuration-and-data/auto-screen-recording.md).
...
and introduce [Ticket metadata](/flutter/configuration-and-data/ticket-metadata.md) (useful key-value pairs) instead. So we did it.
...
There’s no such thing as too many [invocation methods](/flutter/user-feedback/invoke.md).
...
If you want to, Shake's [Activity history](/flutter/configuration-and-data/activity-history.md) now allows you to send yourself any custom logs,
...
By popular demand, Shake can now be [invoked through code](/flutter/user-feedback/invoke/#invoke-through-code) as well.

Submit yourself [silent user feedback](/flutter/user-feedback/silent-user-feedback.md) from the app background,
without interrupting your end user whatsoever!

Privacy options are important. That’s why you now have a method to
disable or enable [Activity history](/flutter/configuration-and-data/activity-history.md) whenever you want to.

If you want to, you can [hide the Inspect button](/flutter/configuration-and-data/new-ticket-screen-elements/#inspect-button) from your users. Brilliant.
